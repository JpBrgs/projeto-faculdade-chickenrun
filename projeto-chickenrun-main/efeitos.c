...
/* Limpa efeitos temporarios ao iniciar ou reiniciar uma partida. */
...
void criarExplosaoParticulas(float x, float y)
{
    ...
    for (i = 0; i < MAX_PARTICULAS; i++) {
        ...
        float forca = 0.20f + (float)(rand() % 80) / 300.0f;
        ...
        particulas[i].corG = 0.75f + (float)(rand() % 25) / 100.0f;
        particulas[i].corB = 0.12f;
        particulas[i].tamanho = 2.0f + (float)(rand() % 4);
...
void atualizarParticulas(float deltaTempo)
{
    ...
    /* O flash diminui com o tempo enquanto particulas perdem vida. */
    if (tempoFlash > 0.0f) {
        tempoFlash -= deltaTempo * 2.8f;
    }
    if (tempoFlash < 0.0f) {
        tempoFlash = 0.0f;
    }
    ...
    for (i = 0; i < MAX_PARTICULAS; i++) {
        ...
        particulas[i].velY -= 0.35f * deltaTempo;
        particulas[i].vida -= deltaTempo * 1.4f;
